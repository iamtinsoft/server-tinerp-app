...
router.get('/', [auth], async (req, res) => {
    ...
    //             (SELECT COUNT(*) FROM ticket_logs tl WHERE tl.ticket_id = t.ticket_id) AS log_count,
    //             c.comment_id,
    //             c.entity_type AS comment_entity_type,
    //             c.entity_id AS comment_entity_id,
    //             c.comment,
    //             c.created_at AS comment_created_at,
    //             ce.first_name AS comment_employee_first_name,
    //             ce.last_name AS comment_employee_last_name,
    //             ce.avatar AS comment_employee_avatar,
    //             l.log_id,
    //             l.action AS log_action,
    //             l.entity_type AS log_entity_type,
    //             l.entity_id AS log_entity_id,
    //             l.performed_at AS log_performed_at,
    //             le.first_name AS log_employee_first_name,
    //             le.last_name AS log_employee_last_name,
    //             le.avatar AS log_employee_avatar,
    //             a.attachment_id,
    //             a.file_path,
    //             a.file_type,
    //             a.file_name,
    //             a.file_size,
    //             a.entity_type AS attachment_entity_type,
    //             a.entity_id AS attachment_entity_id,
    //             a.uploaded_at AS attachment_uploaded_at
    //         FROM tickets t
    //         INNER JOIN tenants tn ON t.tenant_id = tn.tenant_id
    //         INNER JOIN employees e ON t.employee_id = e.employee_id
    //         LEFT JOIN ticket_comments c ON t.ticket_id = c.ticket_id
    //         LEFT JOIN employees ce ON c.entity_id = ce.employee_id -- Join for comment author details
    //         LEFT JOIN ticket_logs l ON t.ticket_id = l.ticket_id
    //         LEFT JOIN employees le ON l.entity_id = le.employee_id -- Join for log author details
    //         LEFT JOIN ticket_attachments a ON t.ticket_id = a.ticket_id
    //         WHERE 1=1
    //         ${ticket_id ? 'AND t.ticket_id = ?' : ''}
    //         ${tenant_id ? 'AND t.tenant_id = ?' : ''}
    //         ${employee_id ? 'AND t.employee_id = ?' : ''}
    ...
    //     const tickets = results.reduce((acc, row) => {
    //         const ticket = acc.find((t) => t.ticket_id === row.ticket_id);
    //         if (!ticket) {
    //             acc.push({
    //                 ticket_id: row.ticket_id,
    //                 title: row.title,
    ...
    //             });
    //         }
    //         const currentTicket = acc.find((t) => t.ticket_id === row.ticket_id);
    ...
    //                 comment: row.comment,
    //                 created_at: row.comment_created_at,
    //                 first_name: row.comment_employee_first_name,
    //                 last_name: row.comment_employee_last_name,
    //                 avatar: row.comment_employee_avatar,
    //             });
    //         }

    //         // Add logs
    //         if (row.log_id) {
    //             currentTicket.logs.push({
    //                 log_id: row.log_id,
    //                 action: row.log_action,
    //                 entity_type: row.log_entity_type,
    //                 entity_id: row.log_entity_id,
    //                 performed_at: row.log_performed_at,
    //                 first_name: row.log_employee_first_name,
    //                 last_name: row.log_employee_last_name,
    //                 avatar: row.log_employee_avatar,
    //             });
    //         }

    //         // Add attachments
    //         if (row.attachment_id) {
    //             currentTicket.attachments.push({
    //                 attachment_id: row.attachment_id,
    //                 file_path: row.file_path,
    //                 file_type: row.file_type,
    //                 file_name: row.file_name,
...
// router.post("/create-log", [auth], async (req, res) => {
//     const { ticket_id,log_id,entity_type,entity_id,comment } = req.body;
//     const db = await mysql.createConnection({
//         host: db_host,
//         user: db_user,
//         password: db_password,
...
//             `INSERT INTO ticket_logs (ticket_id, action, entity_type, entity_id)
//              VALUES (?, ?, ?, ?)`,
//             [ticket_id, "Created Ticket", "Tenant Admin", employee_id]
//         );
